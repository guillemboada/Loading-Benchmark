...
def load_samples(path_list, type, verbose=0):
    samples = []
    if verbose>0:
        for path in tqdm(path_list):
            sample = load_sample(path, type)
...
def generator_from_data(images, masks, batch_size):
    size = len(images)
    i = 0
    while True:
        real_batch_size = min(batch_size, size - i)
...
def define_create_generator_from_data(images, masks, batch_size):
    def create_generator():
        for image, mask in generator_from_data(images, masks, batch_size):
            yield image, mask
    return create_generator
...
def get_generator_from_data(images, masks, batch_size):
    create_generator = define_create_generator_from_data(images, masks, batch_size)
    generator = create_generator()
    return generator
...
def get_dataset_from_data(images, masks, batch_size):
    dataset = tf.data.Dataset.from_generator(define_create_generator_from_data(images, masks, batch_size), output_types=(tf.float32, tf.float32))
    dataset = dataset.repeat()
    dataset = dataset.prefetch(tf.data.AUTOTUNE)
